...
class DmmfDocument {
    constructor({ datamodel, schema, mappings }, options) {
        ...
        this.enums = enumTypes.map(transform_1.transformEnums(this));
        // then transform once again to map the fields (it requires mapped model type names)
        this.models = datamodel.models.map(transform_1.transformModelWithFields(this));
        // transform enums again to map renamed fields
        this.enums = enumTypes.map(transform_1.transformEnums(this));
        this.datamodel = {
            models: this.models,
            enums: datamodel.enums.map(transform_1.transformEnums(this)),
        };
        this.schema = {
            ...transform_1.transformSchema(schema, this),
            enums: this.enums,
        };
        this.modelMappings = transform_1.transformMappings(mappings.modelOperations, this, options);
        this.relationModels = this.models
            .filter(model => model.fields.some(field => field.relationName !== undefined && !field.isOmitted.output))
            .map(transform_1.generateRelationModel(this));
    }
